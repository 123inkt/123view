...
class SearchCodeControllerTest extends AbstractControllerTestCase
{
    ...
    public function testInvokeWithTooShortQuery(): void
    {
        ...
        $request = new Request(['search' => 'fail']);
        $result  = ($this->controller)($request);

        static::assertEquals(['viewModel' => new SearchCodeViewModel([], 'fail')], $result);
    }
    ...
    public function testInvokeWithSearch(): void
    {
        ...
        $this->fileSearcher->expects($this->once())->method('find')->with('success', [$repository])->willReturn($searchResults);

        $request = new Request(['search' => 'success']);
        $result  = ($this->controller)($request);

        static::assertEquals(['viewModel' => new SearchCodeViewModel($searchResults, 'success')], $result);
    }
