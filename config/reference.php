 ...
 *     traceMode?: scalar|null, // The trace mode to use. Either `tracecontext` or `traceid` // Default: "tracecontext"
 *     traceid?: array{
 *         request_header?: scalar|null, // The header in which the bundle will look for and set trace IDs // Default: "X-Trace-Id"
 *         response_header?: scalar|null, // The header the bundle will set the trace ID at in the response // Default: "X-Trace-Id"
 *         generator_service?: scalar|null, // The service name for the trace ID generator. Defaults to `symfony/uid` or `ramsey/uuid`
 *     },
 *     request?: array{
 *         trust_header?: bool, // Whether or not to trust the incoming request's headers as a real TraceID // Default: true
 *         trusted_ips?: scalar|null, // Only trust incoming request's headers if the request comes from one of these IPs (supports ranges/masks). Accepts a string-array, comma separated string or null. Defaults to null, accepting all request IPs. // Default: null
 *     },
 *     response?: array{
 *         send_header?: bool, // Whether or not to send a response header with the trace ID. Defaults to true // Default: true
 *         trusted_ips?: scalar|null, // Only send response if the request comes from one of these IPs (supports ranges/masks) Accepts a string-array, comma or pipe separated string or null. Defaults to null, accepting all request IPs. // Default: null
 *     },
 *     storage_service?: scalar|null, // The service name for trace ID storage. Defaults to `TraceStorage`
 *     monolog?: array{
 *         enabled?: bool, // Whether or not to turn on the trace ID processor for monolog // Default: true
 *     },
 *     console?: array{
 *         enabled?: bool, // Whether to add the trace id to console commands, defaults to true // Default: true
 *         trace_id?: scalar|null, // Option to set the `Trace-Id` to use for console commands from env var. Defaults to null. // Default: null
 *     },
 *     messenger?: array{
 *         enabled?: bool, // Whether to add the trace id to message bus events, defaults to true // Default: true
 *     },
 *     twig?: array{
 *         enabled?: bool, // Whether or not to enable the twig `trace_id()` and `transaction_id()` functions. Only works if TwigBundle is present. // Default: true
 *     },
 *     http_client?: array{
 *         enabled?: bool, // Whether or not to enable the trace id aware http client // Default: true
 *         tag_default_client?: bool, // Whether or not to tag the default http client // Default: false
 *         header?: scalar|null, // The header the bundle set in the request in the http client // Default: "X-Trace-Id"
 *     },
 *     sentry?: array{
 *         enabled?: bool, // Whether or not to enable passing trace and transaction id to Sentry. Note: ensure to set $sentry->tracing->enabled(false) to disable Sentry's own tracing. // Default: false
 *         hub_service?: scalar|null, // The service id of the Sentry Hub. Defaults to Sentry\State\HubInterface // Default: "Sentry\\State\\HubInterface"
 *     },
 * }
 ...
 *     home_route?: scalar|null, // The name of the route to redirect to when clicking the back button
 *     log_files?: array<string, array{ // Default: {"monolog":{"type":"monolog","name":"Monolog","finder":{"in":"%kernel.logs_dir%","name":"*.log","depth":"== 0","ignoreUnreadableDirs":true,"followLinks":false},"downloadable":false,"deletable":false,"start_of_line_pattern":null,"log_message_pattern":null,"date_format":null}}
 *         type?: scalar|null, // The type of log file: monolog, nginx, apache, or the service id of an implementation of `LogFileParserInterface`
 *         name?: scalar|null, // The pretty name to show for these log files
 *         finder: array{
 *             in?: scalar|null, // The symfony/finder pattern to iterate through directories. Example: %kernel.logs_dir%
 *             name?: scalar|null, // The symfony/finder pattern to filter files. Example: *.log // Default: null
 *             depth?: scalar|null, // The symfony/finder directory depth to search files for. Example: '> 0' // Default: null
 *             ignoreUnreadableDirs?: scalar|null, // Whether to ignore unreadable directories // Default: true
 *             followLinks?: scalar|null, // Whether to follow symlinks // Default: false
 *         },
 *         downloadable?: scalar|null, // Whether or not to allow downloading of the log file // Default: false
 *         deletable?: scalar|null, // Whether or not to allow deletion of the log file // Default: false
 *         start_of_line_pattern?: scalar|null, // The regex pattern for the start of a log line. Adds support for multiline log messages. // Default: null
 *         log_message_pattern?: scalar|null, // The regex pattern for a full log message which could include newlines. // Default: null
 *         date_format?: scalar|null, // The date format to parse the date from the log entry. If set to `null`, the format will be guessed by `strtotime` // Default: null
 *     }>,
 *     hosts?: array<string, array{ // Default: {"localhost":{"name":"Local","host":null}}
 *         name?: scalar|null, // The pretty name to show for this host
 *         host?: scalar|null, // The host to connect to // Default: null
 *         auth?: array{
 *             type: scalar|null, // An implementation of AuthenticatorInterface
 ...
 *     view_template?: scalar|null, // Default: null
 *     failure_status_code?: int, // Default: 502
 *     mailer?: bool|array{
 *         enabled?: bool, // Default: false
 *         recipient: list<scalar|null>,
 *         sender: scalar|null,
 *         subject: scalar|null,
 *         send_on_warning?: bool, // Default: true
 *     },
 *     default_group?: scalar|null, // Default: "default"
 *     checks?: array{
 *         groups?: array<string, array{ // Default: []
 *             php_extensions?: list<scalar|null>,
 *             php_flags?: array<string, scalar|null>,
 *             php_version?: array<string, scalar|null>,
 *             process_running?: mixed, // Process name/pid or an array of process names/pids
 *             readable_directory?: list<scalar|null>,
 *             writable_directory?: list<scalar|null>,
 *             class_exists?: list<scalar|null>,
 *             cpu_performance?: scalar|null, // Benchmark CPU performance and return failure if it is below the given ratio
 *             disk_usage?: array{ // Checks to see if the disk usage is below warning/critical percent thresholds
 *                 warning?: int, // Default: 70
 *                 critical?: int, // Default: 90
 *                 path?: scalar|null, // Default: "%kernel.cache_dir%"
 *             },
 *             symfony_requirements?: array{ // Checks Symfony2 requirements file
 *                 file?: scalar|null, // Default: "%kernel.root_dir%/SymfonyRequirements.php"
 *             },
 ...
 *                 configuration_file?: scalar|null, // Absolute path to doctrine migrations configuration
 *                 connection: scalar|null, // Connection name from doctrine DBAL configuration
 *             }>,
 *             memcache?: array<string, array{ // Default: []
 *                 host?: scalar|null, // Default: "localhost"
 ...
 *                 content?: scalar|null, // Default: null
 *             }>,
 *             guzzle_http_service?: array<string, array{ // Default: []
 *                 url?: scalar|null, // Default: "localhost"
 *                 headers?: mixed, // Default: []
 *                 options?: mixed, // Default: []
 *                 status_code?: int, // Default: 200
 *                 content?: scalar|null, // Default: null
 *                 method?: scalar|null, // Default: "GET"
 *                 body?: scalar|null, // Default: null
 *             }>,
 *             rabbit_mq?: array<string, array{ // Default: []
 *                 host?: scalar|null, // Default: "localhost"
 *                 port?: int, // Default: 5672
 *                 user?: scalar|null, // Default: "guest"
 *                 password?: scalar|null, // Default: "guest"
 *                 vhost?: scalar|null, // Default: "/"
 *                 dsn?: scalar|null, // Default: null
 *             }>,
 *             symfony_version?: bool, // Checks the version of this app against the latest stable release
 *             custom_error_pages?: array{ // Checks if error pages have been customized for given error codes
 *                 error_codes: list<scalar|null>,
 *                 path?: scalar|null, // The directory where your custom error page twig templates are located. Keep as "%kernel.project_dir%" to use default location. // Default: "%kernel.project_dir%"
 *                 controller?: scalar|null, // Deprecated: The custom error page controller option is no longer used; the corresponding config parameter was deprecated in 2.13 and will be dropped in 3.0. // Default: null
 *             },
 ...
 *             },
 *             stream_wrapper_exists?: list<scalar|null>,
 *             file_ini?: list<scalar|null>,
 *             file_json?: list<scalar|null>,
 *             file_xml?: list<scalar|null>,
 *             file_yaml?: list<scalar|null>,
 ...
 *                 label: scalar|null,
 *                 warning_expression?: scalar|null, // Default: null
 *                 critical_expression?: scalar|null, // Default: null
 *                 warning_message?: scalar|null, // Default: null
 *                 critical_message?: scalar|null, // Default: null
 *             }>,
 *         }>,
 *     },
 * }
 ...
 *     symfony_trace?: SymfonyTraceConfig,
